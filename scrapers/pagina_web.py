...
(function(){
  ...
  function seta(delta){
    if (delta > 2)    return {s:'\u21C8', cl:'#1D9E75'};
    if (delta > 0.5)  return {s:'\u2191', cl:'#1D9E75'};
    if (delta >= -0.5)return {s:'\u2192', cl:'#888780'};
    if (delta >= -2)  return {s:'\u2193', cl:'#D85A30'};
    return {s:'\u21CA', cl:'#C0392B'};
  }
  ...
  legendaEl.innerHTML = 'A seta acima de cada linha resume a <b style="font-weight:500;color:#666">tendência nas últimas 4 semanas</b> (faixa cinza): ' +
